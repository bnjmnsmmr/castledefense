...
function damageEnemy(e, dmg, source) {
  ...
  if (e.hp <= 0) {
    ...
    if (e.boss) {
      ...
      game.gold += Math.round(bounty * relicMult('gold') * modGoldMult());
      game.activeBoss = null;
      triggerShake(12, 0.7);
      SFX.play('boss_die');
    ...
    game.gold += def.reward * relicMult('gold') * stormGoldMult() * modGoldMult();
    game.kills++;
...
function collectCrate(tx, ty) {
  ...
  game.gold += crate.gold * (hasRelic('magnet') ? 2 : 1) * modGoldMult();
