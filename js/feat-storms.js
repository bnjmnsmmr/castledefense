...
// `game.wave` is the per-world wave counter (js/game.js resets it to 1 whenever a world
// is cleared, at DIFFICULTY.wavesPerWorld — see the wave-complete handler in update()).
// With the default wavesPerWorld (15) that counter never climbs past 30, so this file only
// engages when a long single world is configured (admin panel: raise "Waves per World").
// From wave 31 on, waves are grouped into deterministic 5-wave STORMS: a themed enemy mix
// plus a run-wide mutator. Everything here is seeded off game.runSeed (or the Daily
// Challenge's game.dailySeed) mixed with an integer key via mulberry32 (js/render-world.js)
// — never Math.random() — so a given seed always produces the same sequence of storms.
...
function activeStorm() { return (typeof game !== 'undefined' && game) ? stormFor(game.wave) : null; }
...
// --- MINI-BOSS (called from js/waves.js spawnWave(), only on worlds too short to ever
// reach the storms — i.e. it's a no-op unless game.wave is already past 30) ---
